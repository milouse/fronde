...
describe 'With blog settings' do
  describe 'with working org files' do
    ...
    describe 'with recursive config' do
      before(:each) do
        Neruda::Config.load_test(
          'title' => 'Blog',
          'author' => 'Test',
      ...
      it 'should have generated two indexes', core: true do
      ...
      it 'should generate the right org-header' do
      ...
      describe 'when trying to save them' do
        ...
        it 'should correctly saved one blog index', core: true do
          @index.send(:write_all_blog_home, false)
          expect(File.exist?('writings/index.org')).to be(true)
          expect(IO.read('writings/index.org')).to eq(SAMPLE_PROJECT_INDEX.strip)
        end

        it 'should correctly saved one blog index, even verbosely', core: true do
          @index.send(:write_all_blog_home, true)
          expect(File.exist?('writings/index.org')).to be(true)
          expect(IO.read('writings/index.org')).to eq(SAMPLE_PROJECT_INDEX.strip)
        end
    ...
    describe 'without recursive config' do
      before(:each) do
        Neruda::Config.load_test(
          'title' => 'Blog',
          'author' => 'Test',
      ...
      it 'should have generated one index', core: true do
  ...
  describe 'without org files' do
    ...
    it 'should have generated no index', core: true do
...
describe 'Without blog settings' do
  ...
  it 'should not have generated any indexes', core: true do
    expect(@index.entries.length).to eq(0)
    expect(@index.empty?).to be(true)
  end
...
describe 'Without neither blog settings nor site title' do
  before(:each) do
    FileUtils.mkdir_p 'tmp/txt/writings'
    FileUtils.mkdir_p 'tmp/txt/output'
  ...
  it 'should not save any index', core: true do
    @index.write_org('index')
    expect(File.exist?('tags/index.org')).to be(false)
    @index.write_atom('index')
    expect(File.exist?('output/feeds/index.xml')).to be(false)
